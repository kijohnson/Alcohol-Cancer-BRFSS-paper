...
# trying to reproduce Grucza's result
- use raw data
```{r}

BRFSS <- BRFSS %>%
  mutate(
    binge_binary = case_when(
      (`_RFBING5` == 2 & year == 2016) ~ 1,
      (`_RFBING5` == 1 & year == 2016) ~ 0,
      TRUE ~ NA_real_
    )
  )

design_2016 <- svydesign(
  id = ~1,
  strata = ~`_STSTR`,
  weights = ~`_LLCPWT`,
  data = BRFSS %>% filter(year == 2016)
)

svymean(~binge_binary, design_2016, na.rm = TRUE) # 16.9%

BRFSS_2016 <- BRFSS %>%
  filter(year == 2016)

mean(BRFSS_2016$binge_binary, na.rm= TRUE) # 12.9%
table1(~factor(`_RFBING5`), BRFSS_2016 ) # 12.2%

test <- BRFSS %>%
  filter(year == 2016, !is.na(`_RFBING5`), `_RFBING5` != 9, drinker30== "Yes") %>%
  mutate(
    binge_binary = case_when(
      `_RFBING5` == 2 ~ 1,
      `_RFBING5` == 1 ~ 0,
...
svymean(~binge_binary, design_2016b, na.rm = TRUE) # 16.9% 

table1(~factor(`_RFBING5`), test) # 12.9%
```

```{r}

table(df$DRNK3GE5, useNA = "always")
df <- BRFSS_2016 %>%
  mutate(binge_grucza_style =  case_when(
  DRNK3GE5 >= 1 & DRNK3GE5 <= 76 ~ "Binge drinker",
    DRNK3GE5 == 88 ~ "Not binge drinker",
    DRNK3GE5 %in% c(77, 99) ~ NA_character_
  ))

table(df$binge_grucza_style)

design_2016c <- svydesign(
  id = ~1,
  strata = ~`_STSTR`,
  weights = ~`_LLCPWT`,
  data = df
...
svymean(~binge_grucza_style, design_2016c, na.rm = TRUE) #

```

## this is for checking with Gruzca paper because binge drinking prevalence is different
```{r chunk2}
# 2016
BRFSS_2016 <- read_xpt("../BRFSS data/LLCP2016.XPT ", col_select = c("_STATE", SEX, ALCDAY5, AVEDRNK2, DRNK3GE5, MAXDRNKS, CHCOCNCR,  "_LLCPWT", "_STSTR", "_PSU",  "_RFBING5", "_RFDRHV5", "_AGE_G", "_RACE", HLTHPLN1, EDUCA,"_RFSMOK3", "MARITAL", "_INCOMG" ,"PERSDOC2", "DRNK3GE5"))
BRFSS_2016 <- BRFSS_2016 %>%
  mutate(year = 2016)
```

## Check prevalence according to what is on CDC website
```{r}
# Recode _RFDRHV5 to binary: 1 = heavy drinker, 0 = not heavy drinker
BRFSS_2016 <- BRFSS_2016 %>%
  mutate(binge_drinker = case_when(
    `_RFBING5` == 2 ~ 1,
    `_RFBING5` == 1 ~ 0,
    TRUE ~ NA_real_
  ))

# Calculate unweighted prevalence for each state
state_unweighted_prev <- BRFSS_2016 %>%
  filter(!is.na(binge_drinker)) %>%
  group_by(`_STATE`) %>%
  summarise(
    prevalence = mean(binge_drinker)
  )

# Median prevalence across states
median_unweighted_prev <- median(state_unweighted_prev$prevalence, na.rm = TRUE)

# Output
print(median_unweighted_prev)

# get weighted prevalence:
# 2. Create survey design object
design <- svydesign(
  id = ~1,  # no clustering in public-use BRFSS
  strata = ~`_STSTR`,
  weights = ~`_LLCPWT`,
  data = BRFSS_2016
)

# 3. Get weighted prevalence of heavy drinking by state
state_weighted_prev <- svyby(
  ~binge_drinker,
  ~`_STATE`,
  design,
  svymean,
...
# 4. Calculate the median of the state-level weighted prevalence
median_weighted_prev <- median(state_weighted_prev$binge_drinker, na.rm = TRUE)

# 5. Output
print(median_weighted_prev) # this matches BRFSS crude (they say it's not weighted but I think it is and emailed them about that); heavy drinking also matches BRFSS too (https://www.cdc.gov/brfss/brfssprevalence/index.html)
```
